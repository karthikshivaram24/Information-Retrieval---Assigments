...
class Index(object):
    ...
    def compute_doc_norms(self, index, n_docs, doc_freqs):
        ...
        doc_norm ={}
        for term in index:
            list_tuple = index[term]
            for tup in list_tuple:
                td_idf_term = (1 + math.log10(tup[1])) * (math.log10(n_docs / doc_freqs[term]))
                if tup[0] not in docterm_tfidf:
                    docterm_tfidf[tup[0]] = [(term,td_idf_term)]
                else:
                    docterm_tfidf[tup[0]].append((term,td_idf_term))

        for doc_id in docterm_tfidf:
            sum_weights = 0.0
            tup_list = docterm_tfidf[doc_id]
            for tup in tup_list:
                sum_weights += tup[1] ** 2
            doc_norm[doc_id] = math.sqrt(sum_weights)
...
doc_freq = index.doc_freqs
        ...
        docid_termfreq = {}
        for term  in  index:
            list_docid = index[term]
            for i in list_docid:
                if i[0] not in docid_termfreq:
                    docid_termfreq[i[0]] = [i[1]]
                else:
                    docid_termfreq[i[0]].append(i[1])

        doc_length ={}

        sum_sumld = 0.0
        for doc_id in docid_termfreq:
            sum_ld = 0.0
            for freq in docid_termfreq[doc_id]:
                sum_ld += freq
            doc_length[doc_id] = sum_ld
            sum_sumld += sum_ld

        mean = sum_sumld/len(docid_termfreq)

        return doc_length,mean
    ...
    def create_tf_index(self, docs, doc_freqs):
        ...
        for term in term_doc:
            if term not in term_freq_map:
                term_freq_map[term] = []
            for doc_id in term_doc[term]:
                count_map = Counter(docs[doc_id])
                term_freq_map[term].append([doc_id+1,float(count_map[term])])
    ...
    def count_doc_frequencies(self, docs):
        ...
        for key in doc_freq_dummy:
            if key not in actual_doc_freq:
                actual_doc_freq[key] = float(len(doc_freq_dummy[key]))
    ...
    def query_to_vector(self, query_terms):
        ...
        for term in query_terms:
            if term in doc_freq :
                #print(doc_freq[term])
                if doc_freq[term] > 0:
                    idf = math.log10(n/(float(doc_freq[term])))
                else:
                    idf = 0
