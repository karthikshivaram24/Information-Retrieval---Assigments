...
def parse_query_strings(strings):
    ...
    result_temp = {}
    result = {}
    clean_strings = [line for line in strings if len(line)>1]
    idee = 0
    for line in clean_strings:
        if line.startswith("*FIND"):
            idee = int(re.sub("[^0-9]","",line))
            result_temp[idee] = []
        elif line.startswith("*STOP"):
            break
        else:
            result_temp[idee].append(line)

    for i in result_temp:
        result[i] = " ".join(result_temp[i]).strip() + " "
...
def parse_document_strings(strings):
    ...
    for i in sorted(result_temp.keys()):
        temp = " ".join(result_temp[i]).strip() + " "
...
def search(query, scorer, index):
    ...
    #ind = index.Index()
    #sc = scorer.ScoringFunction()
    query_tokens = index.tokenize(query)
    query_vector = index.query_to_vector(query_tokens)
    result = scorer.score(query_vector,index)

    rounded_result = {}


    rounded_result = {k:round(v,6) for k, v in result.items()}

    #print(rounded_result)


    return sorted(rounded_result,key=result.__getitem__,reverse = True)
